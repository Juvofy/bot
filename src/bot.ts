...
import MissingPermissionsError from "./permissions-error.ts";
			...
			if (timestamps.has(interaction.user.id)) {
				...
				if (now < expirationTime) {
					...
					await interaction.reply({
						content: `Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${interaction.commandName
							}\` command.`,
						ephemeral: true,
					});
					return;

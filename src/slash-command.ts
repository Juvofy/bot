...
class SlashCommand<O extends SlashCommand.Options[] = []> {
	 ...
	 */
	public constructor(data: SlashCommand.Data, fn: typeof this.fn, ...options: O) {
		this.data = data;
		this.fn = fn;
...
declare namespace SlashCommand {
	...
	type OptionsInterfaceMap = {
		[K in keyof SharedSlashCommandOptions<never> as K extends `add${infer R}Option`
		? R
		: never]: SharedSlashCommandOptions<never>[K] extends (builder: (a: any) => infer A) => any
		? A
		: never;
	};
